...
export async function GET(request: Request) {
  ...
  const media = await db.prepare(`SELECT id,url,filename FROM media_assets WHERE deleted_at IS NULL ORDER BY datetime(created_at) DESC LIMIT 100`).all<{id:string;url:string;filename:string|null}>()

...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  const [posts, categories, tags] = await Promise.all([
    ...
    db.prepare(`SELECT c.slug, MAX(p.updated_at) AS updatedAt FROM categories c JOIN post_categories pc ON pc.category_id=c.id JOIN posts p ON p.id=pc.post_id WHERE c.deleted_at IS NULL AND p.status='PUBLISHED' AND p.deleted_at IS NULL GROUP BY c.slug`).all<{ slug: string; updatedAt: string | null }>(),

...
import { Loader2, Menu, Moon, Search, Sun, X } from 'lucide-react'
      ...
      <header className={`mag-header${scrolled ? ' is-scrolled' : ''}`}>
        <div className="mag-container mag-header-inner">
          ...
          <div className="mag-tools">
            <button className="mag-icon-btn" aria-label="Search" onClick={() => setSearchOpen(true)}>
              <Search size={18} strokeWidth={1.9} />
            </button>
            <button className="mag-icon-btn" aria-label="Toggle theme" onClick={toggleTheme}>
              {dark ? <Sun size={18} strokeWidth={1.9} /> : <Moon size={18} strokeWidth={1.9} />}
            </button>
          </div>
        </div>
      </header>
      ...
      {drawer && (
        <div className="mag-drawer is-open" role="dialog" aria-modal="true">
          ...
          <div className="mag-drawer-panel">
            ...
            {categories.map((c) => (
              <Link key={c.id} href={`/category/${c.slug}/`} onClick={() => setDrawer(false)}>{c.name}</Link>
            ))}
          </div>
        </div>
      )}
  ...
  return (
    <div className="mag-search" onMouseDown={onClose}>
      <div className="mag-search-box" onMouseDown={(e) => e.stopPropagation()}>
        ...
        {results.length > 0 && (
          <div className="mag-search-results">
            {results.map((r) => (
              <Link key={r.slug} href={`/${r.slug}/`} className="mag-search-item" onClick={onClose}>
                ...
                <span style={{ minWidth: 0 }}>
                  <h4>{r.title}</h4>
                  <span>{r.categoryName || 'Journal'}</span>
                </span>
              </Link>
            ))}
          </div>
        )}
        {touched && !loading && !results.length && q.trim() && (
          <div className="mag-search-results"><div className="mag-search-empty">“{q}” — কোনো ফলাফল পাওয়া যায়নি।</div></div>
        )}
      </div>
    </div>
  )

...
export default async function EditPostPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="admin-wrap"><div className="admin-shell"><aside className="admin-side"><Link className="brand" href="/admin/">SUHANUR RAHMAN / CMS</Link><nav className="admin-nav"><Link href="/admin/">Dashboard</Link><Link href="/admin/posts/new/">New post</Link><Link href="/admin/taxonomy/">Categories & tags</Link><Link href="/admin/media/">Media</Link><Link href="/admin/comments/">Comments</Link><Link href="/admin/trash/">Trash</Link><Link href={`/${post.slug}/`}>View article ↗</Link></nav></aside><main className="admin-main"><p className="kicker">Publishing</p><h1 className="serif">Edit post</h1><ProPostEditor categories={categories} tags={allTags.results} initial={{ ...editorPost, categoryIds: selected.results.map(x => x.id), tags: tags.results.map(x => x.name) }} /></main></div></div>
}

...
export default async function EditPostPage({params}:{params:Promise<{id:string}>}){
 ...
 const post=await db.prepare(`SELECT id,title,slug,excerpt,content,cover_image AS coverImage,status,seo_title AS seoTitle,seo_description AS seoDescription,seo_keywords AS seoKeywords,og_image AS ogImage,canonical_url AS canonicalUrl,noindex FROM posts WHERE id=? LIMIT 1`).bind(id).first<any>()
 if(!post) return <main className="admin-main"><h1 className="serif">Post not found</h1></main>
 const categories=await getCategories(); const selected=await db.prepare(`SELECT category_id AS id FROM post_categories WHERE post_id=?`).bind(id).all<{id:string}>(); const tags=await db.prepare(`SELECT t.id,t.name FROM tags t JOIN post_tags pt ON pt.tag_id=t.id WHERE pt.post_id=? AND t.deleted_at IS NULL ORDER BY t.name`).bind(id).all<{id:string;name:string}>(); const allTags=await db.prepare(`SELECT id,name FROM tags WHERE deleted_at IS NULL ORDER BY name`).all<{id:string;name:string}>()

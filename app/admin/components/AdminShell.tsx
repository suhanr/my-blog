...
function LogoTile({ className }: { className?: string }) {
  return (
    <span className={cn('grid size-9 place-items-center rounded-xl bg-primary text-sm font-bold text-primary-foreground shadow-xs', className)}>
      SR
    </span>
  )
}
...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  const isActive = (item: NavItem) =>
    item.match === '/admin' ? pathname === '/admin' : pathname === item.match || pathname.startsWith(item.match + '/')
      ...
      <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-border bg-card md:flex">
        <div className="flex items-center gap-3 px-5 py-4">
          ...
          <span className="flex flex-col leading-tight">
            <span className="text-sm font-semibold tracking-tight">Suhanur Rahman</span>
            <span className="text-xs text-muted-foreground">CMS workspace</span>
          </span>
        </div>
        ...
        <div className="p-3">
          <div className="rounded-xl border border-border bg-muted/40 px-4 py-3">
            <p className="flex items-center gap-2 text-[13px] font-medium text-foreground">
              <span className="size-2 rounded-full bg-success" /> Cloudflare
            </p>
            <p className="mt-1 text-xs text-muted-foreground">D1 database · R2 media</p>
          </div>
        </div>
      </aside>
      ...
      <div className="flex min-h-screen flex-col md:pl-64">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-3 border-b border-border bg-card/80 px-4 backdrop-blur md:px-6">
          ...
          <div className="ml-auto flex items-center gap-3">
            ...
            >
              View site <ExternalLink className="size-4" strokeWidth={1.75} />
            </Link>

            <div className="flex items-center gap-2.5 rounded-full py-1 pl-1 pr-1.5">
              <span className="grid size-8 place-items-center rounded-full bg-primary/10 text-xs font-semibold text-primary ring-1 ring-border">SR</span>
              <span className="hidden flex-col leading-tight sm:flex">
                <span className="text-[13px] font-semibold tracking-tight">Suhanur Rahman</span>
        ...
        {/* Mobile nav */}
        <nav className="flex gap-1 overflow-x-auto border-b border-border bg-card px-3 py-2 md:hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {NAV.map((item) => {
            const Icon = item.icon
            const active = isActive(item)
            return (

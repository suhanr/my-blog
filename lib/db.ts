...
export async function listPublishedPosts(limit = 20): Promise<Post[]> {
  const result = await db.prepare(`
    SELECT p.id,p.title,p.slug,p.excerpt,p.content,p.cover_image AS coverImage,p.status,p.published_at AS publishedAt,p.created_at AS createdAt,p.updated_at AS updatedAt,p.category_id AS categoryId,p.seo_title AS seoTitle,p.seo_description AS seoDescription,p.seo_keywords AS seoKeywords,p.og_image AS ogImage,p.canonical_url AS canonicalUrl,p.noindex,
      c.name AS categoryName,c.slug AS categorySlug
    FROM posts p LEFT JOIN categories c ON c.id=p.category_id
    WHERE p.status='PUBLISHED' AND p.published_at IS NOT NULL AND p.deleted_at IS NULL AND (c.deleted_at IS NULL OR c.id IS NULL)
    ORDER BY datetime(p.published_at) DESC LIMIT ?`).bind(limit).all<Post>()
  return result.results
}

export async function getPostBySlug(slug: string): Promise<Post | null> {
  return await db.prepare(`SELECT p.id,p.title,p.slug,p.excerpt,p.content,p.cover_image AS coverImage,p.status,p.published_at AS publishedAt,p.created_at AS createdAt,p.updated_at AS updatedAt,p.category_id AS categoryId,p.seo_title AS seoTitle,p.seo_description AS seoDescription,p.seo_keywords AS seoKeywords,p.og_image AS ogImage,p.canonical_url AS canonicalUrl,p.noindex,c.name AS categoryName,c.slug AS categorySlug FROM posts p LEFT JOIN categories c ON c.id=p.category_id WHERE p.slug=? AND p.status='PUBLISHED' AND p.deleted_at IS NULL LIMIT 1`).bind(slug).first<Post>()
}
...
export async function getCategoryBySlug(slug: string) {
  ...
  const posts = await db.prepare(`SELECT p.id,p.title,p.slug,p.excerpt,p.cover_image AS coverImage,p.published_at AS publishedAt,c.name AS categoryName,c.slug AS categorySlug FROM posts p JOIN categories c ON c.id=p.category_id WHERE c.slug=? AND c.deleted_at IS NULL AND p.status='PUBLISHED' AND p.deleted_at IS NULL ORDER BY datetime(p.published_at) DESC`).bind(slug).all<Post>()
